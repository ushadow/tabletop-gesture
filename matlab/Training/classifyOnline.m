...
function [totalTrueGestures, recRate] = classifyOnline(data, hmm, hmmSeg, trueLabelFiles, filteredIndices)
close all;
...
for i = 1 : n,
  figure(i);
  if (nargin >= 4)
    [numTrueGestures, numCorrect] = classifyOnlineSingle(data{i}, hmm, hmmSeg, trueLabelFiles{i}, filteredIndices{i})
...
function [numTrueGestures, numCorrect] = classifyOnlineSingle(data, hmm, hmmSeg, trueLabelFile, filteredIndices)
...
for i = 1 : length(trueLabels.labels),
  ...
  loglik(t, :) = log(scale);
  for t = (t + 1) : trueLabels.endTimes(i),
...
for n = 1 : numGestures,
  plot(1 : T, loglik(:, n), STYLES{n}, 'lineWidth', 1.2);
  hold 'on';
end
...
for t = 1 : T - 1,
  ...
  if (start ~= 0 && stop ~= 0)
    ...
    style(2) = 'o';
    plot([start stop], [-2300 -2300], STYLES{currentGesture(start)}, 'lineWidth', 2);
    plot([start, stop], [-2300, -2300], style, 'lineWidth', 2);
    start = 0;
    stop = 0;
  end
...
if (start ~= 0)
  style = STYLES{currentGesture(start)};
  style(2) = 'o';
  plot([start T], [-2300 -2300], STYLES{currentGesture(start)}, 'lineWidth', 2);
  plot([start, T], [-2300, -2300], style, 'lineWidth', 2);
end
...
if (nargin >= 4)
  ...
  for i = 1 : length(trueLabels.labels),
    ...
    plot([trueLabels.startTimes(i) trueLabels.endTimes(i)], [-2000 -2000], STYLES{index}, 'lineWidth', 2);
    style = STYLES{index};
    style(2) = 'x';
    plot([trueLabels.startTimes(i), trueLabels.endTimes(i)], [-2000, -2000], style, 'lineWidth', 2);
  end
